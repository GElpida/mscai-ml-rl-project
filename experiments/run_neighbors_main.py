...
class RunConfig:
    episodes: int = 5000
    horizon: int = 50  # rounds per episode
    ...
    gif_frame_duration: float = 0.6  # seconds per frame (exploration phase)

    # Visualize "exploration -> exploitation" change in the GIF
    exploit_start_episode: int = 4000  # 1-based episode index used for visualization
    gif_every_exploit: int = 10        # frame every N episodes after exploit_start_episode
    gif_frame_duration_exploit: float = 1.0  # seconds per frame after exploit_start_episode
...
def run_neighbors(cfg: RunConfig):
    ...
    for ep in range(cfg.episodes):
        ...
        # GIF frame (always include final episode so you can see episode cfg.episodes)
        ep_idx = ep + 1  # 1-based for display
        in_exploit = ep_idx >= cfg.exploit_start_episode
        if in_exploit:
            take_frame = ((ep_idx - cfg.exploit_start_episode) % cfg.gif_every_exploit) == 0
            frame_duration = cfg.gif_frame_duration_exploit
        else:
            take_frame = (ep % cfg.gif_every) == 0
            frame_duration = cfg.gif_frame_duration

        if take_frame or (ep == cfg.episodes - 1):
            gif_frames_data.append((ep_idx, list(last_actions_profile)))
...
def plot_results(results: dict, cfg: RunConfig):
    ...
    # Avg reward per episode (by type, separate figures)
    plt.figure()
    plt.plot(results["avg_reward_ep_type"]["X"])
    plt.title("Average reward per episode (Type X)")
    plt.xlabel("Episode")
    plt.ylabel("Avg reward")
    save_fig(out_dir, f"{p}_avg_reward_per_episode_type_X.png")

    plt.figure()
    plt.plot(results["avg_reward_ep_type"]["Y"])
    plt.title("Average reward per episode (Type Y)")
    plt.xlabel("Episode")
    plt.ylabel("Avg reward")
...
def make_network_gif(results: dict, cfg: RunConfig):
    ...
    durations = results.get("gif_durations")

    n = len(results["agent_types"])

    try:
        from scripts.build_graph import build_graph, default_positions

        game = make_default_game_7()
        G = build_graph(game)
        node_labels = {node_id: G.nodes[node_id]["label"] for node_id in range(n)}
        pos = default_positions(node_labels) or nx.spring_layout(G, seed=7)
    except Exception:
        G = nx.Graph()
        G.add_nodes_from(range(n))
        for i in range(n):
            for j in results["adjacency"][i]:
                if i < j:
                    G.add_edge(i, j)
    ...
    cmap = ListedColormap(["#1f77b4", "#ff7f0e"])  # action 0, action 1
    legend_handles = [
        Patch(facecolor="#1f77b4", edgecolor="black", label="action 1"),
        Patch(facecolor="#ff7f0e", edgecolor="black", label="action 2"),
    ]

    for ep_idx, actions_profile in frames_data:
        phase = "exploitation" if ep_idx >= cfg.exploit_start_episode else "exploration"
        if ep_idx == cfg.exploit_start_episode:
    ...
    imageio.mimsave(gif_path, images, duration=durations or cfg.gif_frame_duration)
