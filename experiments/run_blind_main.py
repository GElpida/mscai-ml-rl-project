...
Action = int
State = Tuple  # (last_action, last_reward, agent_type)
...
class RunConfig:
    ...
    # ✅ your folder (use raw string or forward slashes)
...
# -------------------------
# Run (your run_blind can stay as you had it)
# Just ensure it returns:
# results["gif_frames_data"] = [(ep, p_action2_list), ...]
# results["agent_types"] = list(game.agent_types)
# results["adjacency"] = game.adjacency
# -------------------------
def run_blind(cfg: RunConfig):
    ...
    last_reward: List[float] = [0.0] * game.n_agents

    tracked_ids = {"X1": 0, "Y1": 2, "X3": 4}
    q_hist: Dict[str, List[Tuple[float, float]]] = {k: [] for k in tracked_ids}
    ref_state_by_agent: Dict[int, State] = {i: (0, 0.0, game.agent_types[i]) for i in range(game.n_agents)}
    ...
    for ep in range(cfg.episodes):
        ...
        for t in range(cfg.horizon):
            states: List[State] = [
                (last_action[i], last_reward[i], game.agent_types[i]) for i in range(game.n_agents)
            ]
        ...
        for name, aid in tracked_ids.items():
            ref_s = ref_state_by_agent[aid]
            q = agents[aid].Q.get(ref_s, [0.0, 0.0])
        ...
        if (ep + 1) % 500 == 0:
            print(f"episode {ep+1}/{cfg.episodes} | eps={eps:.2f} | avgR={avg_all:.3f} | nashconv={nashconv_ep[-1]:.3f}")
...
def save_fig(out_dir: Path, filename: str):
    plt.tight_layout()
...
def plot_results(results: dict, cfg: RunConfig):
    ...
    for name, series in results["q_hist"].items():
        ...
        plt.title(f"{name}: Q-values per episode (blind)")
        plt.xlabel("Episode")
        plt.ylabel("Q")
        plt.legend()
...
# -------------------------
# Network GIF (proper adjacency visualization)
# -------------------------
def make_network_gif(results: dict, cfg: RunConfig):
    ...
    for ep, p_action2 in frames_data:
        ...
        # Node "intensity" = p(action2)
        # (matplotlib will color-map these; we avoid hardcoding colors)
        node_values = p_action2

        nx.draw_networkx_edges(G, pos, alpha=0.5)
        nx.draw_networkx_nodes(
            G, pos,
            node_size=900,
            node_color=node_values,  # mapped
            vmin=0.0, vmax=1.0
        )
